...
CREATE TABLE Tasks (
    ...
    Status VARCHAR(20) NOT NULL DEFAULT 'todo' COMMENT '状态：todo, in-progress, review, done',
...
BEGIN
    SELECT 
        ...
        u.Email,
        u.IsActive,
        COALESCE(GROUP_CONCAT(g.Name ORDER BY g.Name SEPARATOR ','), '') as Groups
    FROM Users u
    LEFT JOIN UserGroupMemberships ugm ON u.Id = ugm.UserId
    LEFT JOIN UserGroups g ON ugm.GroupId = g.Id
...
('Design Homepage UI/UX', 'Create wireframes, mockups and user experience flows for the main landing page', 'todo', 'high', 'John Doe', '2025-07-20', 16, 
 '["Design", "Frontend", "UX"]', 
 '{"responsible": ["John Doe"], "accountable": ["Carol Johnson"], "consulted": ["Jane Smith", "Bob Wilson"], "informed": ["Alice Brown", "Charlie Davis"]}',
...
('Database Schema Migration', 'Configure PostgreSQL database with new user authentication tables and relationships', 'in-progress', 'medium', 'Jane Smith', '2025-07-18', 12,
 ...
 '{"responsible": ["Jane Smith"], "accountable": ["Carol Johnson"], "consulted": ["John Doe", "Bob Wilson"], "informed": ["Alice Brown", "Admin User"]}',
 (SELECT Id FROM UserGroups WHERE Name = 'Development'),
 'Jane Smith',
 'Carol Johnson',
 'John Doe,Bob Wilson',
...
('User Authentication System', 'Implement secure login system with JWT tokens and multi-factor authentication', 'review', 'high', 'Bob Wilson', '2025-07-16', 20,
 ...
 '{"responsible": ["Bob Wilson"], "accountable": ["Admin User"], "consulted": ["Carol Johnson", "Jane Smith"], "informed": ["John Doe", "Alice Brown"]}',
...
('API Documentation Portal', 'Document all REST endpoints with interactive examples and testing interface', 'done', 'low', 'Alice Brown', '2025-07-15', 8,
 ...
 '{"responsible": ["Alice Brown"], "accountable": ["Carol Johnson"], "consulted": ["John Doe", "Jane Smith"], "informed": ["Bob Wilson", "Charlie Davis"]}',
...
('Mobile Responsive Layout', 'Optimize application layout for mobile devices and tablets with touch interactions', 'todo', 'medium', 'Charlie Davis', '2025-07-25', 14,
 ...
 '{"responsible": ["Charlie Davis"], "accountable": ["Carol Johnson"], "consulted": ["Jane Smith", "John Doe"], "informed": ["Alice Brown", "Bob Wilson"]}',
...
SELECT 'Carol用户组分配检查' as CheckType, 
       ...
       u.Name,
       u.Email,
       COALESCE(GROUP_CONCAT(g.Name ORDER BY g.Name SEPARATOR ', '), 'No Groups') as Groups
FROM Users u
LEFT JOIN UserGroupMemberships ugm ON u.Id = ugm.UserId
LEFT JOIN UserGroups g ON ugm.GroupId = g.Id
